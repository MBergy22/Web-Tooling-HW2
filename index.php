...
// Validation function to check if a string contains only alphanumeric characters
...
// Validation function to check if an email has a valid format
...
if (isset($_POST['login'])) {
    ...
    if (empty($userNameError) && empty($passwordError)) {
        // Rest of your code for database query and password verification...
    } else {
...
if (isset($_POST['login'])) {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        // Verifying Password
        if (password_verify($password, $hashpass) && ($Role == 'User' || $Role == 'Admin' || $Role == 'SuperAdmin')) {
...
if (isset($_POST['login'])) {
    ...
    // Get data from the database, column names (basically means OR)
    // Get data from the database, matching the username
    $query = "SELECT UserName, UserEmail, LoginPassword, Role FROM userdata WHERE UserName=:uname";
    ...
    if ($stmt->rowCount() > 0) {
        ...
        if (password_verify($password, $hashpass) && ($Role == 'User' || $Role == 'Admin' || $Role == 'SuperAdmin')) {
            ...
            if ($Role == 'Admin') {
                ...
                header("Location: superadmin.php"); // Redirect to the superadmin page
                exit();
            }
        } else {
            $_SESSION['error'] = 'Incorrect username or password. Please try again.';
